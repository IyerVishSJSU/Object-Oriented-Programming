...
public:
    ...
    int i,bit_count,x;
    int init_prod_vec(int s_sval);
    int shift_mul(vector <bool> &m_vec);
    void prod_add();
    int two_comp();
    int feedDecimal(char* s_ival,char* s_sval);
    int feedBit(char* s_ival,char* s_sval);
    void app_bits(int siz_val,int choice);
    ...
    int res_display(int pval,int dval,int hval,int opflag);
};
int ShiftReg::init_prod_vec(int s_sval)
{
    for(i=0;i<s_sval;i++)
    {
...
int ShiftReg::shift_mul(vector <bool> & m_vec)
{
    //cout<<vec<<endl;
    //cout<<m_vec<<endl;
    if(m_vec.back())
    ...
    //cout<<"After Operation:"<<result<<endl<<'\n';
return 0;
}
void ShiftReg::prod_add()
{
    bool sum=0;
    for(i=(vec.size()-1);i>=0;i--)
    {
        sum=vec[i]^result[i]^c;
        c= (vec[i]&result[i])|(result[i]&c)|(vec[i]&c);
        result[i]=sum;
...
int ShiftReg::feedDecimal(char*s_ival,char*s_sval)
{
    ...
    char holder=' ';
    unsigned int reg_size=atoi(s_sval);
...
void ShiftReg::app_bits(int siz_val,int choice)
{
    if(choice==0)
    {
        for(i=0;i<siz_val;i++)
...
int ShiftReg::res_display(int pval,int dval,int hval,int opflag)
{
    if(opflag==0)
    {
        c=0;
...
int main (int argc, char *argv[])
{
    ...
    int sim_idx=0;
    int arg_idx=0;
    while ((c = getopt (argc, argv,"pdhi:s:r:l:I:R:L:v:o:"))!= -1)
        switch (c)
        {
//---------------------------------------------------
        case 'i':
...
if(iflag1!=1&&sflag!=1)
    ...
    else
    {
        ...
        else
        {
            ...
            if(found!=NULL)
            {
                n=sr1.feedDecimal(ival1,sval);
            }
            else
                {
                            n=sr1.feedBit(ival1,sval);
                }
        }
...
/*FUNCTION CALL FOR ADDITION-SUBTRACTION-MULT*/
for(q=0;q<strlen(oval);q++)
	{
		if(oval[q]!=' ')
		{
			operval=oval[q];
...
if(oflag==1)
{
    ...
    case '-':
        ...
        break;
    case 'x':
        sr.init_prod_vec(atoi(sval)*2);

        if(strchr(ival,'D')!=NULL)
        {
            if(strncmp(ival,"-",1)==0)
            {
               sr.app_bits(atoi(sval),1);
            }
            else{
                sr.app_bits(atoi(sval),0);
            }
        }
        else
        {
            sr.app_bits(atoi(sval),2);
        }
        sr1.app_bits(atoi(sval),0);
        /*if(strchr(ival1,'D')!=NULL)
        {
            if(strncmp(ival1,"-",1)==0)
            {
               sr1.app_bits(atoi(sval),1);
            }
            else{
                sr1.app_bits(atoi(sval),0);
            }
        ...
        {
            sr1.app_bits(atoi(sval),2);
        }*/

        for(i=0;i<(atoi(sval)*2);i++)
        {
           n=sr.shift_mul(sr1.vec);
           sr.shiftLeft((char*)"1",0);
           sr1.shiftRight((char*)"1",0);
        }
        break;
    }
    if(n!=0)
...
if(pflag==1)
{
        ...
        if(operval=='+')
        {
          sr.res_display(pflag,dflag,hflag,1);
        }
        if(operval=='-')
        {
          sr.res_display(pflag,dflag,hflag,0);
        }
        if(operval=='x')
        {
          sr.res_display(pflag,dflag,hflag,2);
        }
        return 0;
}
